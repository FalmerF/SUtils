...
public class GuiButtonSound extends GuiElement{
	...
	public void draw(Minecraft mc, int positionX, int positionY)
    {
        if (this.visible)
        {
            ...
            this.drawScaledCustomSizeModalRect(this.posX, this.posY, 0, 0, 1, 1, this.minWidth, this.height, 3, 1);
            this.drawScaledCustomSizeModalRect(this.posX+this.width-this.minWidth, this.posY, 2, 0, 1, 1, this.minWidth, this.height, 3, 1);
            this.drawScaledCustomSizeModalRect(this.posX+this.minWidth, this.posY, 1, 0, 1, 1, this.width-this.minWidth*2, this.height, 3, 1);
    ...
    public void SetSoundName() {
        if(!option.equals("")) {
        	this.displayString = ModuleUtils.soundName[ModuleSettings.getInt(option)];
        }
    }
    ...
    public void clicked(int id) {
    	int sound = ModuleSettings.getInt(option);
    	if(id == 0) {
    		sound++;
    	}
    	else if(id == 1) {
		...
		if(sound >= ModuleUtils.soundName.length) sound = 0;
		else if(sound < 0) sound = ModuleUtils.soundName.length-1;
		ModuleSettings.setParam(option, sound);
		SetSoundName();
    }
}
