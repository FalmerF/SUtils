...
public class GuiReconnecting extends GuiScreen {
    ...
    public GuiReconnecting(GuiScreen parent, String reason, ITextComponent message)
    {
	...
	public void initGui()
    {
        ...
        timer = ModuleSettings.getInt("reconnectTime");
        if(timer == 0) {
        	timer = 1200;
        	ModuleSettings.setParam("reconnectTime", timer);
        }
        reconnect = ModuleSettings.getBool("reconnect2");
    }
	...
	protected void actionPerformed(GuiButton button)
    {
        ...
        {
        	reconnect = !reconnect;
        	ModuleSettings.setParam("reconnect2", reconnect);
        }
        else if (button.id == 2)
        {
        	try {
	...
	public void drawScreen(int p_73863_1_, int p_73863_2_, float p_73863_3_)
    {
        ...
        this.drawCenteredString(this.fontRenderer, "Auto-Reconnect by SMB " + ModuleInfo.version, this.width / 2, this.height - 10, 11184810);
        int var4 = this.height / 2 - 30;

        if (this.multilineMessage != null)
        {
            for (Iterator var5 = this.multilineMessage.iterator(); var5.hasNext(); var4 += this.fontRenderer.FONT_HEIGHT)
            {
