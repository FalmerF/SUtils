...
public class GuiField extends GuiElement {
	...
	public GuiField(int id, int posX, int posY, int width, String description, String option, FieldType fieldType){
		...
		this.option = option;
        if(!this.option.equals("")) this.text = ModuleSettings.getString(this.option);
	...
	public void draw(Minecraft mc, int positionX, int positionY) {
		if(this.visible) {
            ...
            Gui.drawScaledCustomSizeModalRect(this.posX, this.posY, 0, 0, 1, 1, this.minWidth, this.minHeight, 3, 3);
            Gui.drawScaledCustomSizeModalRect(this.posX+this.width-this.minWidth, this.posY, 2, 0, 1, 1, this.minWidth, this.minHeight, 3, 3);
            Gui.drawScaledCustomSizeModalRect(this.posX, this.posY+this.height-this.minHeight, 0, 2, 1, 1, this.minWidth, this.minHeight, 3, 3);
            Gui.drawScaledCustomSizeModalRect(this.posX+this.width-this.minWidth, this.posY+this.height-this.minHeight, 2, 2, 1, 1, this.minWidth, this.minHeight, 3, 3);
            
            Gui.drawScaledCustomSizeModalRect(this.posX+this.minWidth, this.posY, 1, 0, 1, 1, this.width-(this.minWidth*2), this.minHeight, 3, 3);
            Gui.drawScaledCustomSizeModalRect(this.posX+this.minWidth, this.posY+this.height-this.minHeight, 1, 2, 1, 1, this.width-(this.minWidth*2), this.minHeight, 3, 3);
            Gui.drawScaledCustomSizeModalRect(this.posX, this.posY+this.minHeight, 0, 1, 1, 1, this.minWidth, this.height-(this.minHeight*2), 3, 3);
            Gui.drawScaledCustomSizeModalRect(this.posX+this.width-this.minWidth, this.posY+this.minHeight, 2, 1, 1, 1, this.minWidth, this.height-(this.minHeight*2), 3, 3);
            
            Gui.drawScaledCustomSizeModalRect(this.posX+this.minWidth, this.posY+this.minHeight, 1, 1, 1, 1, this.width-(this.minWidth*2), this.height-(this.minHeight*2), 3, 3);
			...
			if(this.selected) {
				...
				if(cursorPos != selectionPos) {
					...
					if(firstCharLine == secondCharLine) {
				        RenderUtil.drawRect(firstPos, lineHeight*firstCharLine-1+this.posY+3+textAnchorY, secondPos, lineHeight*(firstCharLine+1)-2+this.posY+3+textAnchorY, GuiTheme.textSelect);
					}
					else {
						for(int i = firstCharLine; i <= secondCharLine; i++) {
							...
							if(this.posY-10 < linePosY && this.posY+this.height-3 > linePosY) {
								if(i == firstCharLine) {
									RenderUtil.drawRect(firstPos, lineHeight*i-1+this.posY+3+textAnchorY, firstPos+firstSelectedWidth, lineHeight*(i+1)-2+this.posY+3+textAnchorY, GuiTheme.textSelect);
								}
								else if(i == secondCharLine) {
									RenderUtil.drawRect(this.posX+3, lineHeight*i-1+this.posY+3+textAnchorY, secondPos, lineHeight*(i+1)-2+this.posY+3+textAnchorY, GuiTheme.textSelect);
								}
								else {
									RenderUtil.drawRect(this.posX+3, lineHeight*i-1+this.posY+3+textAnchorY, this.posX+3+fontRender.getStringWidth(lines[i]), lineHeight*(i+1)-2+this.posY+3+textAnchorY, GuiTheme.textSelect);
								}
							}
						}
					}
	...
	public void setSelected(boolean selected) {
    	...
    	if(selected) {
    		...
    		if(!this.option.equals("")) {
    			ModuleSettings.setParam(option, this.text);
    		}
    	}
    }
