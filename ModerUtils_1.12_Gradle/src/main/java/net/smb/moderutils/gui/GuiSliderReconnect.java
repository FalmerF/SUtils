...
public class GuiSliderReconnect extends GuiButton
{
    ...
    public GuiSliderReconnect(int id, int posX, int posY, float minValue, float maxValue)
    {
        ...
        this.value = ModuleSettings.getInt("reconnectTime");
        if(this.value == 0) this.value = 1200;
        
        value = (float)((value/20.00F)-10)/290.00F;

        this.displayString = name + ModuleSettings.getInt("reconnectTime")/20; //displayString
    }
    ...
    {
        this.pressed = false;
        ModuleSettings.setParam("reconnectTime", GuiReconnecting.timer);
    }
}
