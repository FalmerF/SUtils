...
public class ScriptActionFancyItem extends ScriptAction
{
    ...
    public IReturnValue execute(final IScriptActionProvider provider, final IMacro macro, final IMacroAction instance, final String rawParams, final String[] params) {	
    	...
    	if(params.length >= 3) {
	    	...
	    	final Object theWorld = AbstractionLayer.getWorld();
	    	MovingObjectPosition movingObj = mc.objectMouseOver;
	    	TileEntity tileBlock =  ((WorldClient)theWorld).getTileEntity(movingObj.blockX, movingObj.blockY, movingObj.blockZ);
	    	...
	    	try {
	    		itemnum = (int)clamp(Integer.parseInt(ScriptCore.parseVars(provider, macro, params[0], false))-1,0,1);
	    		posX = (int)clamp(Integer.parseInt(ScriptCore.parseVars(provider, macro, params[1], false)),-750,750);
	    		posY = (int)clamp(Integer.parseInt(ScriptCore.parseVars(provider, macro, params[2], false)),-750,750);
	    	} catch(Exception e) {
	    		return null;
	    	}
	    	...
	    	if(params.length >= 4) {
	    		try {
	    			int scale = (int)clamp(Integer.parseInt(ScriptCore.parseVars(provider, macro, params[3], false))-1,0,8);
	    			if(itemnum == 0) {
