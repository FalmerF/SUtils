...
import net.smb.sutils.common.FunctionDescriptions;
import net.smb.sutils.common.GuiTheme;
import net.smb.sutils.effects.EffectBase;
import net.smb.sutils.gui.elements.*;
...
public class GuiScreenModerMenu extends GuiScreen {
	...
	public void initGui()
    {
		if(SettingsModule.getInt("guiPosX") == 0) {
			this.posX = (this.width/2)-231;
			this.posY = (this.height/2)-140;
			SettingsModule.setParam("guiPosX", this.posX);
			SettingsModule.setParam("guiPosY", this.posY);
		}
		else {
			this.posX = SettingsModule.getInt("guiPosX");
			this.posY = SettingsModule.getInt("guiPosY");
		}
        ...
        if(currentHint <= 9) {
	        // Start Hint
	        hint = new GuiHint(49, 156, 102, 150, 65, "Добро пожаловать в SUtils!\nДанный гайд поможет Вам\nосвоиться в нашем модуле\n\n§e(Кликните по подсказке)");
	        this.hints.add(hint);
	        
	        // Player Category Hint
	        hint = new GuiHint(50, 125, buttonPos-10, 150, 50, "Все функции распределены\nпо категориям.\nВ §ePlayer§r находятся\nфункции для игроков.");
	        hint.setRegion(15, buttonPos, 125, buttonPos+20);
	        this.hints.add(hint);
	        
	        // Category region Hint
	        hint = new GuiHint(51, 24, buttonPos-10, 114, 40, "Внутри категорий\nВы сможете найти\nвсе нужные функции.");
	        hint.setRegion(135, 50, 425, 255);
	        this.hints.add(hint);
	        
	        // Info Hint
	        hint = new GuiHint(49, 147, 41, 150, 50, "У каждой функции есть\nсвоё описание, для его\nотображения наведите\nкурсор на §6(i)§r.");
	        hint.setRegion(138, 51, 146, 59);
	        this.hints.add(hint);
	        
	        // Formating Codes Hint
	        hint = new GuiHint(49, 29, 225, 172, 40, "Некоторые функции используют\nкоды форматирования,\nпосмотреть их Вы можете тут.");
	        hint.setRegion(20, 253, 28, 261);
	        this.hints.add(hint);
	        
	        // Settings Hint
	        hint = new GuiHint(49, 279, 30, 155, 50, "В настройках Вы можете\nизменить клавиши открытия\nменю и заметок, а также\nработа с конфигами.");
	        hint.setRegion(415, 21, 424, 30);
	        this.hints.add(hint);
	        
	        // Radio Hint
	        hint = new GuiHint(52, 305, 31, 132, 30, "Тебе скучно и одиноко?\nЕ***шь українский рок!");
	        hint.setRegion(395, 19, 407, 31);
	        this.hints.add(hint);
	        
	        // Radio Hint 2
	        hint = new GuiHint(49, 130, 75, 145, 40, "Для добавления своего\nрадио Вам нужна прямая\nссылка на поток.");
	        hint.setRegion(140, 115, 425, 130);
	        this.hints.add(hint);
	        
	        // Notes Hint
	        hint = new GuiHint(53, 156, 102, 150, 40, "Открыть меню §eЗаметок§r\nВы можете нажав клавишу\n§eHome§r (по стандарту).");
	        this.hints.add(hint);
        }
        ...
        boolean chatFilterEnabled = SettingsModule.getBool("chatFilter");
        ...
        for(int i = 0; i < 10; i++) {
        	if(!SettingsModule.getString("autoADText" + i).equals("")) {
        ...
        func.elements.add(new GuiTextString(20, 55, 0.8F, "Рекламируйте модуль и получите §6VIP§r бесплатно!\n"
        		...
        		+ " • Отправив рекламу §l"+Utils.getDataString("adsCount")+" раз§r Вы получите §6SUtils VIP§r на §l2д§r.\n\n"
        ...
        if(!SettingsModule.getBool("stats")) buttonStandart.enabled = false;
        ...
        if(SettingsModule.getString("statsPattern").equals("")) {
        	SettingsModule.setParam("statsPattern", "Баланс: &6%money%");
        }
        ...
        field.enabled = SettingsModule.getBool("playerNotify");
        func.elements.add(field);
        ...
        field.enabled = SettingsModule.getBool("friendNotify");
        func.elements.add(field);
        ...
        func = new GuiFunction(-1, 0, 1050, 380, "Удаление не нужных сообщений");
        func.width = 320;
        ...
        if(ModuleInfo.permission >= 1) {
	        ...
	        toggle.enabled = SettingsModule.getBool("autoScreen");
	        func.elements.add(toggle);
	        ...
	        if(SettingsModule.getString("moderChatCustomCommand").equals("")) SettingsModule.setParam("moderChatCustomCommand", "/c");
        ...
        if(ModuleInfo.permission >= 3) {
	        ...
	        for(Entry<String, String> param : Utils.data.entrySet()) {
        ...
        buttonConfigName = SettingsModule.config.getName();
        ...
        String text = Utils.getDataString("StartScreen").replaceAll("\\\\n", "\n");
        ...
        if(ModuleInfo.vip) {
	        ...
	        text = Utils.getDataString("VipScreen").replaceAll("\\\\n", "\n");
        ...
        if(!ModuleInfo.vip) {
	        ...
	        text = Utils.getDataString("VipSaleScreen").replaceAll("\\\\n", "\n");
        ...
        guiText = new GuiTextString(150, 35, 0.8F, String.valueOf(SettingsModule.radios.keySet().toArray()[SettingsModule.getInt("currentRadio")]));
        ...
        this.setPosAll();
        setEffect(SettingsModule.getInt("effect"));
	...
	public void updateScreen() {
		...
		this.headPosX = (int)this.lerp(this.headPosX, this.posX+25, 0.3F);
        this.headPosY = (int)this.lerp(this.headPosY, this.posY+40, 0.3F);
	...
	protected void actionPerfomed(GuiElement element) {
		...
		switch (element.id)
        {
			...
			case 25:
				...
				break;
			case 26:
				List<String> configs = SettingsModule.GetAllConfigs();
				...
				break;
			case 27:
				SettingsModule.SetCurrentConfig(buttonConfigName.replaceAll("\\.config", ""));
				this.initGui();
				break;
			case 29:
				String name = ((GuiField)this.getById(28)).getText();
				name = SettingsModule.createConfig(name.replaceAll("\\.config", ""));
				SettingsModule.SetCurrentConfig(name.replaceAll("\\.config", ""));
				this.initGui();
				break;
			case 30:
				if(SettingsModule.deleteConfig(buttonConfigName.replaceAll("\\.config", ""))) {
					this.initGui();
				}
				else {
			...
			case 32:
                ...
                break;
			case 33:
				GuiToggle toggle = (GuiToggle) element;
				if(toggle.active) {
					this.mc.thePlayer.addPotionEffect(new PotionEffect(Potion.nightVision.getId(), Integer.MAX_VALUE));
				}
				else {
					this.mc.thePlayer.removePotionEffectClient(Potion.nightVision.getId());
				}
				break;
			case 34:
			...
			case 36:
		        ...
		        SettingsModule.setParam("statsPos", posId);
				break;
			case 37:
			...
			case 41:
				if(Radio.canPlay) {
			        ...
			        if(Radio.radio == null || !Radio.radio.isPlaying()) {
			        	...
			        	Radio.playRadio(String.valueOf(SettingsModule.radios.keySet().toArray()[SettingsModule.getInt("currentRadio")]));
			        }
			        else {
			...
			case 44:
				if(Radio.canPlay) {
					num = SettingsModule.getInt("currentRadio");
					num = num-1 < 0 ? SettingsModule.radios.size()-1 : num-1;
					Radio.playRadio(String.valueOf(SettingsModule.radios.keySet().toArray()[num]));
				}
				break;
			case 45:
				if(Radio.canPlay) {
					num = SettingsModule.getInt("currentRadio");
					num = num+1 > SettingsModule.radios.size()-1 ? 0 : num+1;
					Radio.playRadio(String.valueOf(SettingsModule.radios.keySet().toArray()[num]));
				}
				break;
			case 46:
				if(Radio.canPlay) {
					...
					if(!fieldName.getText().equals("") && !fieldLink.getText().equals("")) {
						SettingsModule.radios.put(fieldName.getText(), fieldLink.getText());
						SettingsModule.saveRadios();
			...
			case 62:
			{
				this.getById(63).enabled = ((GuiToggle)element).active;
				this.getById(64).enabled = ((GuiToggle)element).active;
				break;
			}
        }
	}
	...
	public void updateRadioText() {
		((GuiTextString)getById(49)).SetText(String.valueOf(SettingsModule.radios.keySet().toArray()[SettingsModule.getInt("currentRadio")]));
	}
	...
	public void setEffect(int effectNum) {
        ...
        if(effect != null) {
        	...
        	if(!effect.vip || (effect.vip && ModuleInfo.vip)) {
        		SettingsModule.setParam("effect", effectNum);
        	}
        	else if(SettingsModule.getInt("effect") == effectNum) SettingsModule.setParam("effect", 0);
        }
        else SettingsModule.setParam("effect", 0);
	}
	
	protected void actionReleased(GuiElement element) {
		if(element.id == 43) {
			if(Radio.radio != null) Radio.radio.setVolume(SettingsModule.getFloat("radioVolume")/100);
		}
	}
	...
	public void drawScreen(int posX, int posY, float partialTicks) // Draw Screen
    {
    	...
    	this.func_152125_a(this.posX, this.posY, 0, 0, 1, 1, (int)(420*1.1F), (int)(255*1.1F), 1, 1);
    	
    	mc.getTextureManager().bindTexture(AbstractionLayer.getPlayer().getLocationSkin());
        
    	this.func_152125_a(this.headPosX, this.headPosY, 1, 1, 1, 1, 20, 20, 8, 4);
    	
    	if(this.holiday == 1) {
    		mc.getTextureManager().bindTexture(this.ribbon);
	    	this.func_152125_a(this.headPosX-10, this.headPosY+9, 0, 0, 1, 1, 25, 25, 1, 1);
    	}
    	if(ModuleInfo.vip) {
	    	mc.getTextureManager().bindTexture(this.vip);
	    	this.func_152125_a(this.posX+25, this.posY+61, 0, 0, 1, 1, 20, 11, 1, 1);
    	}
    	...
    	GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
    	this.func_152125_a(this.posX + 15, this.posY + 15, 1, 1, 1, 1, 20, 20, 1, 1);
    	
    	if(ModuleInfo.playerName.length() > 13) {
    		GL11.glPushMatrix();
        	GL11.glTranslatef(this.posX + 50, this.posY + 41, 0.0F);
            GL11.glScalef(0.8F, 0.8F, 0.8F);
            fontRender.drawString(ModuleInfo.playerName, 0, 0, GuiTheme.textColor);
        	GL11.glPopMatrix();
    	}
    	...
    	this.func_152125_a(this.posX + 137, this.posY + 20, 0, 0, 1, 1, 6, 10, 1, 1);
    	
    	if(SettingsModule.getBool("osFilter") && ModuleInfo.permission > 0) {
        	...
        	this.func_152125_a(this.posX + 222, this.posY + 20, 0, 0, 1, 1, 6, 10, 1, 1);
    	}
    	
		if(SettingsModule.getBool("tpsFilter") && ModuleInfo.permission > 1) {
        	...
        	this.func_152125_a(this.posX + 275, this.posY + 20, 0, 0, 1, 1, 10, 8, 1, 1);
		}
		GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
	...
	protected void mouseMovedOrUp(int posX, int posY, int button) // mouseMovedOrUp
    {
    ...
    public void handleInput() // handleInput
    {
        if (Mouse.isCreated())
        {
            while (Mouse.next())
