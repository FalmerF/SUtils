...
public class GuiSlider extends GuiElement{
    ...
    public GuiSlider(int id, int posX, int posY, int width, float minVal, float maxVal, String option)
    {
        ...
        this.option = option;
        try {
        if(!this.option.equals("")) this.value = Float.parseFloat(SettingsModule.getString(option));
        } catch(Exception e) {
        	this.value = 0.8F;
        	SettingsModule.setParam(option, (float)(this.maxVal-this.minVal)*this.value);
        	e.printStackTrace();
        }
	...
	public void draw(Minecraft mc, int positionX, int positionY)
    {
        if (this.visible)
        {
            ...
            this.func_152125_a(this.posX, this.posY+4, 0, 0, 1, 1, this.width, 2, 1, 1);
            
            // Render Handle
            mc.getTextureManager().bindTexture(handleTexture);
            this.func_152125_a((int)(this.posX+((this.width-6)*this.value)), this.posY+2, 0, 0, 1, 1, 6, 6, 1, 1);
            
            FontRenderer fontRender = mc.fontRenderer;
            
            // Render Value Field
            mc.getTextureManager().bindTexture(GuiTheme.sliderValue);
            this.func_152125_a((int)(this.posX+this.width+4), this.posY, 0, 0, 1, 1, 20, 10, 1, 1);
            // Render Value Text
    ...
    public void released(int p_146118_1_, int p_146118_2_) {
    	...
    	if(!this.option.equals("")) SettingsModule.setParam(option, (float)(this.maxVal-this.minVal)*this.value);
    }
}
