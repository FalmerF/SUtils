...
public class GuiToggle extends GuiElement{
    ...
    public GuiToggle(int id, int posX, int posY, String option)
    {
    	...
    	this.option = option;
    	if(!this.option.equals("")) this.active = SettingsModule.getBool(this.option);
    }
	...
	public void draw(Minecraft mc, int positionX, int positionY)
    {
        if (this.visible)
        {
            ...
            this.func_152125_a((int)this.currentPosX, this.posY-1, 0, 0, 1, 1, 7, 7, 1, 1);
        }
    }
    ...
    public void clicked() {
    	if(this.enabled && this.visible) {
    		active = !active;
    		if(!this.option.equals("")) SettingsModule.setParam(option, this.active);
    	}
    }
}
